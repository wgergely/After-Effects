...
var makelocal = true;
var makelocal_prefix = "shadowLight";
...
function apply(name, makelocal, makelocal_prefix){
	...
	if (check)
	{
		...
		if (makelocal)
		{
			...
			//ADD CUSTOM ACTION HERE TO MODIFY THE LOCALISED MATERIAL
			...
			SIConnectShaderToCnxPoint(opacity, d + ".standard.opacity", false);
			
			//Depth
			/*
			RemoveAllShadersFromCnxPoint(d + ".utility.color", siShaderCnxPointBasePorts);
			SetValue(d + ".utility.color.blue", 0, null);
			SetValue(d + ".utility.color.green", 0, null);
			SetValue(d + ".utility.color.red", 0, null);
			SetValue(d + ".utility.shade_mode", "flat", null);
			*/
		}
		return true
	}
