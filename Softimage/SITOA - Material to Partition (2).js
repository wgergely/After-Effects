...
//INIT
function init(){
	var paramSet=ActiveSceneRoot.AddProperty("CustomProperty",false,"Material_to_Partition") ;
	...
	{	
        ...
        function addButtons(){
            xsiLayout.AddRow();
                ...
                xsiLayout.AddButton("rename","Rename");
        ...
        function addMaterials(){
            ...
            PPG.AddParameter("filter", siString ) ;
            
            xsiLayout.AddGroup( "", false, 100 );
                var filter = xsiLayout.AddItem( "filter", "", "String" ) ;
                filter.SetAttribute( "NoLabel", true ) ;
            xsiLayout.EndGroup();
            xsiLayout.AddGroup( "", false, 100 );
             ...
             xsiLayout.EndGroup();
            AddMaterialsToList(xsiLayout, filter_array(getMaterialNames(), ["standard_", "uv_", "motion_", "flat_", "ao_", "Scene_Material", "\_", "depth_"]).sort(natSort))
        }
	}
    ...
    function AddMaterialsToList(PPGLayout, in_Array)
    {
        ...
        populateList(PPGLayout, "matList", in_Array.sort(natSort));
    }
	...
	paramSet.PPGLayout.Logic = init_partition.toString() + "\n;" +
        ...
        rename_OnClicked.toString() + "\n;" +
        select_OnClicked.toString() + "\n;" +
        newpreset_OnClicked.toString() + "\n;" +
        refreshMatList_OnClicked.toString() + "\n;" +
        AddMaterialsToList.toString() + "\n;" +
        getMaterialNames.toString() + "\n;" +
        filter_array.toString() + "\n;" +
        containsString.toString() + "\n;" +
        matList_OnChanged.toString() + "\n;" +
        filter_OnChanged.toString();
    
	paramSet.PPGLayout.Language = "JScript";
	function passes_OnChanged()
	{
        var prefs = Application.Preferences;
        var cmdlog = prefs.GetPreferenceValue("scripting.cmdlog");
        var msglog = prefs.GetPreferenceValue("scripting.msglog");
        var autoinspect = prefs.GetPreferenceValue("Interaction.autoinspect");
        prefs.SetPreferenceValue("scripting.cmdlog", false); 
        prefs.SetPreferenceValue("scripting.msglog", false);
        ...
        prefs.SetPreferenceValue("scripting.cmdlog", cmdlog);
        prefs.SetPreferenceValue("scripting.msglog", msglog);
        prefs.SetPreferenceValue("Interaction.autoinspect", autoinspect);
	}
	function newpass_OnClicked()
	{
        ...
        var materials = filter_array(getMaterialNames(), ["standard_", "uv_", "motion_", "flat_", "ao_", "Scene_Material", "\_", "depth_"]);
        AddMaterialsToList(PPG.Inspected.Item(0).PPGLayout, containsString(materials, PPG.filter.Value));
	...
	function create_OnClicked()
	{
        ...
        if (PPG.matList.value.split(";").length > 0){
        SelectObj(PPG.matList.value.split(";"));
            var PPG_properties = {
	...
	function rename_OnClicked()
	{
        ...
        var s = PPG.matList.value.split(";");
        var matName = "";
        for (var i = 0; i < s.length; i++)
        {
            matName = s[i].slice(s[i].lastIndexOf('.') + 1);
            libName = s[i].slice(0, s[i].lastIndexOf('.'));
            
            newName = rename( matName );
            SetValue(libName + "." + matName + ".name", newName);
            
            try{ SetValue(libName + "." + "motion_" + matName + ".name", "motion_" + newName); } catch(e){};
            try{ SetValue(libName + "." + "flat_" + matName + ".name", "flat_" + newName); }catch(e){};
            try{ SetValue(libName + "." + "uv_" + matName + ".name", "uv_" + newName); }catch(e){};
            try{ SetValue(libName + "." + "standard_" + matName + ".name", "standard_" +  newName); }catch(e){};
            try{ SetValue(libName + "." + "ao_" + matName + ".name", "ao_" + newName); }catch(e){};
            
            try{ SetValue( Application.ActiveProject.ActiveScene.ActivePass.fullname + "." + matName + ".name", newName); }catch(e){};   
        }
        ...
        function rename( oldName )
        {
            ...
            prop.AddParameter2( "newname", siString, oldName );
            var xsiLayout = prop.PPGLayout;

            prop.Name = "Rename Material" ;
            xsiLayout.AddStaticText( "Enter New Name:") ;
            var newname = xsiLayout.AddItem( "newname", "", "String" ) ;
            newname.SetAttribute( "NoLabel", true ) ;

            var inspect = InspectObj( prop, "", "Rename Material", siModal, false );
            if(!inspect){
                return prop.newname.value;
            } else {
                return oldName;
            }
        }
    ...
    function newpreset_OnClicked()
    {
        ...
        CopyPaste("Material_to_Partition", null, "temp01234", 2);
        CopyPaste("temp01234.Material_to_Partition", null, "Scene_Root", 1);
        DeleteObj("temp01234");
    ...
    function containsString(in_Array, term) {
        var match, return_Array = in_Array;
        var regexp = new RegExp(term);
        for (var i = in_Array.length - 1; i >= 0; i--) {
            match = return_Array[i].match(regexp);
            if (!match){
...
try {
	var prop = Dictionary.GetObject( "Material_to_Partition", false );
	if (!prop)
	{
		prop = init();
