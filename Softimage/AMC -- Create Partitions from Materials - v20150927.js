...
function init(){
	...
	{	
        ...
        xsiLayout.AddRow();
            xsiLayout.AddStaticText(' Current Pass is  "' + ActiveProject.ActiveScene.ActivePass.name + '"  (' + ActiveProject.ActiveScene.ActiveMaterialLibrary.name + ')');
        xsiLayout.EndRow();
        xsiLayout.AddSpacer( 10,5 )
        addButtons();
        xsiLayout.AddSpacer( 10,5 )
        ...
        function addButtons(){
            xsiLayout.AddRow();
                xsiLayout.AddButton("create","> Add Partition");
...
/*            var cS = application.selection;
            ...
            for ( i = 0 ; i < in_Array.length ; i++ )
            {
                ...
                var count_string = "";
                if (count == 0){count_string = "Not used by any objects";} else { count_string = "Used by '" + count + "' objects";}
	...
	function delete_OnClicked()
	{
		...
		if (check_selection())
		{
            ...
            {
                try{DeleteObj(Application.ActiveProject.ActiveScene.ActivePass.fullname + "." + application.selection(j).name)} catch(e){logMessage(e),siErrorMsg}
            }
		} else {
			XSIUIToolkit.MsgBox( "Invalid selection. \nPlease select only materials.", siMsgExclamation , "Create Material Based Partitions")
		};
