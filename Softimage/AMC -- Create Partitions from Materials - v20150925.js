...
function init(){
	...
	{	
        ...
        function addButtons(){
            xsiLayout.AddRow();
                xsiLayout.AddButton("create","> Make Partition <");
                xsiLayout.AddButton("delete","> Remove Partition <");
                xsiLayout.AddButton("newpreset","Duplicate Property");
                xsiLayout.AddButton("refreshMatList","Refresh List");
            xsiLayout.EndRow();
        }
    ...
    function AddMaterialsToList(PPGLayout)
    {
        function matte_state(in_PartitionName){
            var b = Dictionary.GetObject(Application.ActiveProject.ActiveScene.ActivePass.fullname + ".Background_Objects_Partition",false)
            var a;
        ...
        {
            //Get Number of Objects Using the Material
/*            var cS = application.selection;
            var cS_arr = [];
            for (var w = 0; w < cS.count; w++){
                cS_arr.push(cS(w).fullname);
            }*/
            // Populate the specified list
            var aItems = new Array( in_Array.length * 2 ) ;
            for ( i = 0 ; i < in_Array.length ; i++ )
            {
                SelectObjectsUsingMaterial(matLib + '.' + in_Array[i]);
                var count = application.Selection.count;                
                ...
                if (check){
                    if (matte_state())
                    {
                        aItems[i * 2] = " > " + in_Array[i] + " <  / Matte ON / Used " + count + " Times)";
                    } else {
                        aItems[i * 2] = " > " + in_Array[i] + " < / Used " + count + " Times /";
                    }
                } else {
                    aItems[i * 2] = in_Array[i] + "  / Used " + count + " Times /";
                }
        ...
        for (var x = 0; x < matLib_items.count; x++) { materials.push(matLib_items(x).name) }
        ...
        populateList(PPGLayout, "matList", filter_array(materials, ["standard_", "uv_", "motion_", "flat_", "ao_", "Scene_Material", "\_"]).sort(natSort));
    }
	...
	function delete_OnClicked()
	{
		...
		if (check_selection())
		{
            ...
            {
                try{DeleteObj(Application.ActiveProject.ActiveScene.ActivePass.fullname + "." + application.selection(j).name)} catch(e){logMessage(e),siErr}
            }
		} else {
			XSIUIToolkit.MsgBox( "Invalid selection. \nPlease select only materials.", siMsgExclamation , "Create Material Based Partitions")
		};
