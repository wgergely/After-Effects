...
function ExportSelected(x) {
    ...
    function callExport(outputPath, subdirectory, quality,width,height,inFrame,outFrame) {
        var idExpr = charIDToTypeID( "Expr" );
            var desc1 = new ActionDescriptor();
            var idUsng = charIDToTypeID( "Usng" );
                var desc2 = new ActionDescriptor();
                ...
                var idsubdirectory = stringIDToTypeID( "subdirectory" );
                desc2.putString( idsubdirectory, subdirectory );

                var idNm = charIDToTypeID( "Nm  " );
                desc2.putString( idNm, subdirectory +"""_.png""" );
    ...
    function exportPNG(){
        ...
        //Check if there's a background layer, indexes seem to be reversed if there is a Background Layer!
        ...
        try{
            ...
            for (j=0;j< selectedLayers.length;j++){
                //Name = index_name
                var name = selectedLayers[j]+'_'+ getLayerNameByIndex( selectedLayers[j] );

                makeActiveByIndex(selectedLayers[ls.length-j],true); //index is reversed!

                toggleIsolateLayer();
                try {callExport(dExportRootPath,name,3,2880,1620,inFrame,outFrame);} catch(e){}
                toggleIsolateLayer();

                //DEBUG
                //alert('i=' + selectedLayers[j] +' of '+ls.length+'\nn='+getLayerNameByIndex( selectedLayers[j] )+'\nactive='+l);
            }
        } catch(e){
            //No Background layer
            for (i=0;i<selectedLayers.length;i++){
                var name = selectedLayers[i]+'_'+ getLayerNameByIndex( selectedLayers[i] );
                makeActiveByIndex(selectedLayers[i],true);

                toggleIsolateLayer();
                try {callExport(dExportRootPath,name,3,width,height,inFrame,outFrame);state=true} catch(e){ toggleIsolateLayer();return false};
                toggleIsolateLayer();

                //DEBUG
                //alert('i=' + selectedLayers[i] +' of '+(ls.length+1)+'\nn='+getLayerNameByIndex( selectedLayers[i] )+'\nactive='+l);
