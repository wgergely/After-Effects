...
I was originally intending this to be a cross pltform tool but it only has been implemented to work with windows. Has not been tested on Windows 10.
...
var RenderQueuePlus = (function ( thisObj ) {
    ...
    function importFootage( inPath, sequence, compName, version ){
        ...
        //folders
        {
            ...
            for ( var i = 1; i <= app.project.rootFolder.items.length; i++ ) {
                 if ( app.project.rootFolder.item( i ).name === 'prerenders' ) {
                    rExists = true;
                    r = app.project.rootFolder.item( i );
                    break;
                }
            }
            if (!rExists){
        ...
        for ( var i = 1; i <= v.items.length; i++ ) {
            if ( v.item( i ).name === item.name ) {
                Window.alert('Footage already exists in the project.','Render Queue+',undefined)
                item.remove();
                item = v.item( i );
                break;
            }
    ...
    //Settings Object
    {
        ...
        settings.lastmodified = '26/02/2016';
        settings.version = '0.1.11';
    ...
    var FrameWindow = ( function () {
        //Extensions, Window
        {
            ...
            function setinfo( window, inInfo1, inInfo2, inInfo3, inInfo4 ) {
                var infoField1 = window.findElement('infoField1'),
                    infoField2 = window.findElement('infoField2'),
                    infoField3 = window.findElement('infoField3'),
                    infoField4 = window.findElement('infoField4');
            ...
            function setlist( window, inNumColumns, inColumn1, inColumn2, inColumn3 ){
                var item,
                    listGroup = window.findElement('listGroup'),
                    listItem = window.findElement('listItem'),
                    searchGroup = window.findElement('searchGroup');
            ...
            function makeWindow( inTitle, inNumColumns, columnTitles ) {
                var columnWidths = [(initX/6)*3.5,(initX/6)*1.5,(initX/6)-25]
                ...
                //Events
                {
                        ...
                        if ( listItem.selection ) {
                            ...
                            if (choice) {
                                for (var i = 0; i < selected.length; i++) {
                                    ...
                                    var indx = data.item( index ).exists.names.indexOf( selected[i].text );
                                    while ( !file.changePath( data.item( index ).exists.fsNames[ indx ] )){
                                    ...
                                    if ( file.exists )
                                    {
                                        ...
                                        } else {  Window.alert( 'Sorry, there was an error deleteing the file.\nUnkown error.', 'Render Queue+' )}
                                    } else {
                                        Window.alert( 'Sorry, there was an error deleteing the file.\nFile doesn\'t exist.', 'Render Queue+')
                                    }
                                }
                            }
                        } else {
                            Window.alert('Select an item from the list below before continuing.')
                        }
                    };
                    ...
                    function refreshButton_onClick() {
                        ...
                        //Clear and reset listItem
                        var listItem = w.findElement('listItem');
                            listItem.removeAll();
                            setlist( w, 3, data.item( index ).exists.names, data.item( index ).exists.dates, data.item( index ).exists.sizes );

                        //Clear and reset main Window
                        var cs = mainWindow.getSelection();
                        mainWindow.clear();
                        mainWindow.setlist( data.compnames, data.filenames, data.rendered.frames, data.missing.frames, data.incomplete.frames, data.rendered.sizes );
                    ...
                    function importButton_onClick() {
                        ...
                        if ( listItem.selection ) {
                            for (var i = 0; i < selected.length; i++) {
                                ...
                                try {
                                    importFootage( data.item( index ).exists.fsNames[ idx ], false, data.item( index ).compname, pathcontrol.getVersionString() );
                    ...
                    function incompleteButton_onClick() {
                        ...
                        if (state == false) {
                            state = true;
                            setlist( w, 3, data.item( index ).incomplete.names, data.item( index ).incomplete.dates, data.item( index ).incomplete.sizes );
                        } else {
                            state = false;
                            setlist( w, 3, data.item( index ).exists.names, data.item( index ).exists.dates, data.item( index ).exists.sizes );
                        };
                ...
                //UI
                {
                    ...
                    // Buttons
                    {
                        var controlsGroup = w.add('group',undefined,{
                            name: 'headerGroup',
                            orientation: 'row'
                        });
                        ...
                        var sep = controlsGroup.add('iconbutton',undefined, MainWindow_Sep,{
                            style: 'toolbutton'
                        });
                            sep.enabled = false;
                            sep.size = [1,elemSize];

                        var browseButton = controlsGroup.add('iconbutton',undefined, MainWindow_RevealIcon, {
                            name: 'browseButton',
                            style: 'toolbutton'
                        });
                        ...
                        var importButton = controlsGroup.add('iconbutton',undefined, MainWindow_ImportIcon, {
                            name: 'importButton',
                            style: 'toolbutton'
                        });
                        ...
                        var incompleteButton = controlsGroup.add('button',undefined, 'Show Incomplete',{
                        ...
                        var refreshButton = controlsGroup.add('iconbutton',undefined, MainWindow_RefreshIcon,{
                            name: 'refreshButton',
                            style: 'toolbutton'
                        });
                    ...
                    var sep1 = w.add('group',undefined,{
                        name: 'sep1',
                        orientation: 'row'
                    });
                    sep1.size = [20,20];
                    ...
                    // Info
                    {
                        var infoGroup4 = w.add('group',undefined,{
                            name: 'infoGroup4',
                            orientation: 'row'
                        });
                        var infoField4 = infoGroup4.add('edittext',undefined,'-',{
                            name: 'infoField4'
                        });
                        var infoGroup1 = w.add('group',undefined,{
                            name: 'infoGroup1',
                            orientation: 'row'
                        });
                        var infoField1 = infoGroup1.add('statictext',undefined,'-',{
                            name: 'infoField1'
                        });
                        var infoGroup2 = w.add('group',undefined,{
                            name: 'infoGroup2',
                            orientation: 'row'
                        });
                        var infoField2 = infoGroup2.add('statictext',undefined,'-',{
                            name: 'infoField2'
                        });
                        var infoGroup3 = w.add('group',undefined,{
                            name: 'infoGroup3',
                            orientation: 'row'
                        });
                        var infoField3 = infoGroup3.add('statictext',undefined,'-',{
                            name: 'infoField3'
                        });
                        var infoGroup5 = w.add('group',undefined,{
                            name: 'infoGroup5',
                            orientation: 'row'
                        });
                        var infoField5 = infoGroup5.add('statictext',undefined,'',{
                            name: 'infoField5'
                        });
                        infoGroup5.enabled = false;

                        infoGroup1.margins = infoGroup2.margins = infoGroup3.margins = infoGroup4.margins = 0;
                        infoGroup1.spacing = infoGroup2.spacing = infoGroup3.spacing = infoGroup4.spacing = 0;
                        infoGroup1.alignChildren = infoGroup2.alignChildren = infoGroup3.alignChildren = infoGroup4.alignChildren = 'left';
                    }
                    // Search
                    {
                    ...
                    //Set default sizes
                    {
                        searchGroup.size = searchField.size = infoGroup1.size = infoGroup2.size = infoGroup3.size = infoGroup4.size = infoField1.size = infoField2.size = infoField3.size = infoField4.size = [initX, 20];
                    ...
                    //On Resize
                    {
                        w.onResizing = w.onResize = function () {
                            searchGroup.size = searchField.size = infoGroup1.size = infoGroup2.size = infoGroup3.size = infoGroup4.size = infoField1.size = infoField2.size = infoField3.size = infoField4.size = [w.size.width, 20];
                            listGroup.size = listItem.size = [w.size.width,500];
                            w.layout.resize();
                        }
                    }
                }
        ...
        //cls
        {
            var cls = function( ){
                ...
                this.show = function ( inTitle, inInfo1, inInfo2, inInfo3, inInfo4, inNumColumns, columnTitles, inColumn1, inColumn2, inColumn3 ) {
                    if (!window) {
                        window = makeWindow(inTitle, inNumColumns, columnTitles );
        ...
        */
        ...
        cls.prototype = {
            callSystem: function ( inPath, args ) {
                ...
                var cmd, stat = {}, stats = [], splitln, lines,
                ...
                if (File.fs === 'Windows') {
                    ...
                    try {
                        ...
                        error2 = raw.indexOf('Logon failure: unknown user name or bad password.');
    ...
    var Data = ( function () {
        ...
        var cls = function (){
            ...
            //main
            this.items = (function() {
                var parent = null, dataObj = {}, dataObjs = [];
                //Collect Data
                for (var i = 1; i <= app.project.renderQueue.numItems; i++) {
                    for (var j = 1; j <= app.project.renderQueue.item(i).numOutputModules; j++) {
                        ...
                        // Obj Definition
                        {
                            ...
                            dataObj.rqItem = app.project.renderQueue.item(i);
                            dataObj.omItem = app.project.renderQueue.item(i).outputModule(j);
                        ...
                        dataObj.set = (function () {
                            ...
                            if ( dataObj.ready && dataObj.omItem.file ){

                                dataObj.padding = getPadding( dataObj.omItem.file.name );
                                dataObj.ext = dataObj.omItem.file.name.slice(-3);
                                if( !dataObj.padding ) {
                                    dataObj.sequencename = decodeURI( decodeURI(dataObj.omItem.file.name)) + ' ' + '[' + pad(dataObj.startframe, dataObj.padding) + '-' + pad(dataObj.endframe, dataObj.padding) + ']';
                                } else {
                                    dataObj.sequencename = decodeURI( decodeURI(dataObj.omItem.file.name).slice(0, ((dataObj.padding + 2 + 4) * (-1)) )) + '[' + pad(dataObj.startframe, dataObj.padding) + '-' + pad(dataObj.endframe, dataObj.padding) + ']' + '.' + dataObj.ext;
                                };

                                dataObj.filename = decodeURI (  dataObj.omItem.file.parent.name ) + sep + decodeURI ( dataObj.sequencename );

                                var stat = new Directory ( dataObj.omItem.file.parent );
                                var files;

                                if ( dataObj.padding > 0 ) {
                                    files = stat.files( '*' + dataObj.ext );
                                } else {
                                    files = stat.files( dataObj.omItem.file.displayName );
                                };
                                ...
                                var set = (function () {
                                    ...
                                    if ( !(files.item(0).name == ( 'Invalid path.' || 'Error.' )) ) {
                                        ...
                                        if ( dataObj.padding > 0) {
                                            //SEQUENCE
                                            for (var i = 0; i < parseInt( dataObj.duration, 10 ); i++ ) {
                                                frame = pad(dataObj.startframe + i, dataObj.padding);
                                                name = decodeURI(dataObj.omItem.file.name).slice(0, (dataObj.padding + 2 + 4) * (-1) ) + frame + '.' + dataObj.ext;
                                                index = names.indexOf( name );
                                                fsName = dataObj.omItem.file.parent.fsName + sep + name;
                                                if ( index >= 0) {
                                        ...
                                        //NON SEQUENCE ITEMS
                                            ...
                                            frame = 1;
                                            name = decodeURI(dataObj.omItem.file.name);
                                            index = names.indexOf( name );
                                            fsName = dataObj.omItem.file.parent.fsName + sep + name;
                                ...
                                dataObj.filname = 'File not yet specified.';
                                dataObj.ext = null;
                                dataObj.padding = null;
        ...
        var cls = function( omItem, rqItem ) {
            ...
            this.apply = function () {
                ...
                if ( settings.getSetting('pathcontrol_fsName') ) {
                    ...
                    try { rendersBase.create(); foldersOK = true; } catch (e) { foldersOK = false; Window.alert('Sorry, Unable to create render base folder:\n\n\n'+e,'Version Control: Error') }
                } else {
                    Window.alert('The output base path has not been set.\nCheck the panel preferences if you have a valid pattern set.\n\nNote: You can still set the destination manually in the Render Queue.','No basepath set.');
                }
    ...
    var MainWindow = ( function ( thisObj, inTitle, inNumColumns, columnTitles, columnWidths ) {
        ...
        cls.prototype = {
            ...
            setlist: function ( inColumn1,inColumn2,inColumn3,inColumn4,inColumn5,inColumn6 ){
                ...
                function statuscolor( index ){
                    if ( (data.item( index ).rendered.count === data.item( index ).duration) && (data.item( index ).incomplete.count === 0) ) {
                       return greenIcon
                    }
                    if ( (data.item( index ).rendered.count === data.item( index ).duration) && (data.item( index ).incomplete.count > 0) ) {
                       return orangeIcon
                    }
                    if ( (data.item( index ).rendered.count < data.item( index ).duration) && (data.item( index ).rendered.count > 0) ) {
                        return orangeIcon;
                    }
                    if ( data.item( index ).incomplete.count > 0 ) {
                        return orangeIcon;
                    }
                    if ( data.item( index ).missing.count === data.item( index ).duration ) {
                        return redIcon;
                    }
                    return grayIcon
                }
            ...
            aerender: function ( promptForFile ) {
                ...
                if ( listItem.selection && saved ) {
                    var index = cls.prototype.getSelection(),
                        batname = app.project.file.displayName + '-' + data.item( index ).compname + ".bat";
                    ...
                    //Archive Comp
                    var comparchive = new Folder ( settings.getSetting('pathcontrol_fsName') + sep + '.aeparchive' + sep + data.item( index ).compname + sep + pathcontrol.getVersionString() );
                    comparchive.create();
                    try { app.project.file.copy( comparchive.fsName + sep + data.item( index ).compname + '_' + pathcontrol.getVersionString() + '_' + app.project.file.displayName )} catch(e) { Window.alert(e) };
                    ...
                    if ( bat ) {
                        ...
                        var start = 'start \"' + 'Rendering ' + data.item( index ).compname + ' of ' + app.project.file.displayName+ '\" ' + cmd;
            ...
            refresh: function() {
                ...
                cls.prototype.clear();
                cls.prototype.setlist( data.compnames, data.filenames, data.rendered.frames, data.missing.frames, data.incomplete.frames, data.rendered.sizes );
        ...
        //Event Functions
        {
            function playButton_onClick() {
                if ( listItem.selection ) {
                    ...
                    if (player === 'rv'){
                        if (rvPath == 'null' || rvpushPath == 'null') { Window.alert('RV or RV Push path is not set. Check preferences.'); return false;}
            ...
            function browseButton_onClick() {
                if ( listItem.selection ) {
                    var index = listItem.selection.index;
                    //Parent
                    if ( data.item( index ).file.parent.exists ) {
                         data.item( index ).file.parent.execute()
                    } else {
                        //Grandparent
                       if ( data.item( index ).file.parent.parent.exits ) {
                            data.item( index ).file.parent.parent.execute()
                       } else {
                           //Great grandparent
                           if ( data.item( index ).file.parent.parent.parent.exists ) {
                                data.item( index ).file.parent.parent.parent.execute()
                           } else {
                               // etc...
                               if ( data.item( index ).file.parent.parent.parent.parent.exists) {
                                    data.item( index ).file.parent.parent.parent.parent.execute()
                               } else {
                                   if ( data.item( index ).file.parent.parent.parent.parent.parent.exists ) {
                                        data.item( index ).file.parent.parent.parent.parent.parent.execute()
                                   }
                               }
                           }
                       }
            ...
            function framesButton_onClick() {
                if ( listItem.selection ) {
                    ...
                    var a = data.item( index ).compname,
                        b = 'Basepath: ' + '\'' + settings.getSetting('pathcontrol_fsName') + '\'',
                        c = 'Composition: ' + data.item( index ).compname,
                        d = 'Version: ' + (function(){if (!pathcontrol.getVersion()){return 'not set';} else {return pathcontrol.getVersionString();}}());
                        e = String( data.item( index ).omItem.file.fsName );
                        f = data.item( index ).exists.names;
                        g = data.item( index ).exists.dates;
                        h = data.item( index ).exists.sizes;

                    frameWindow.show( a, b, c, d, e, 3, ['Name','Date','Size'], f, g, h)
            ...
            function importButton_onClick(){
                if (listItem.selection) {
                    var index = listItem.selection.index;
                    try{
                    importFootage( data.item( index ).exists.fsNames[0], true, data.item( index ).compname, pathcontrol.getVersionString() )
                    }catch(e){Window.alert(e)}
                }
            }
            ...
            function versionsDropdown_onChange(){
                ...
                if ( versionsDropdown.selection.text === 'Set Version Control' ) {
                    pathcontrol = new Pathcontrol( data.item( listItem.selection.index ).omItem, data.item( listItem.selection.index ).rqItem );
                    if (!pathcontrol.getVersion() ) {
                        ...
                        if (settings.getSetting('pathcontrol_fsName')[settings.getSetting('pathcontrol_fsName').length-1] == '/' || settings.getSetting('pathcontrol_fsName')[settings.getSetting('pathcontrol_fsName').length-1] == '\\') {
                            pathcontrol.setBasepath ( settings.getSetting('pathcontrol_fsName') + data.item( listItem.selection.index ).compname );
                        } else {
                            pathcontrol.setBasepath ( settings.getSetting('pathcontrol_fsName') + sep + data.item( listItem.selection.index ).compname )
                        }
        ...
        //UI
        {
            // Header bar
            {
                ...
                //Grp1
                {
                    var aerenderButton = controlsGroup1.add('iconbutton', undefined, MainWindow_StartRender, {
                        name: 'aerenderButton',
                        style: 'toolbutton'
                    });
                    ...
                    var batchButton = controlsGroup1.add('iconbutton',undefined, MainWindow_SaveRender,{
                        name: 'aerenderButton',
                        style: 'toolbutton'
                    });
                    ...
                    var playButton = controlsGroup1.add('iconbutton',undefined, MainWindow_PlayIcon,{
                        name: 'playButton',
                        style: 'toolbutton'
                    });
                    ...
                    var browseButton = controlsGroup1.add('iconbutton',undefined,MainWindow_RevealIcon,{
                        name: 'browseButton',
                        style: 'toolbutton'
                    });
                        browseButton.onClick = browseButton_onClick;
                    ...
                    var framesButton = controlsGroup1.add('iconbutton',undefined,MainWindow_FilesIcon,{
                        name: 'framesButton',
                        style: 'toolbutton'
                    });
                    ...
                    var refreshButton = controlsGroup1.add('iconbutton',undefined,MainWindow_RefreshIcon,{
                        name: 'refreshButton',
                        style: 'toolbutton'
                    });
                        refreshButton.onClick = refreshButton_onClick;
                    ...
                    var settingsButton = controlsGroup1.add('iconbutton',undefined,MainWindow_SettingsIcon,{
                        name: 'settingsButton',
                        style: 'toolbutton'
                    });
                ...
                //grp2
                {
                    ...
                    var versionsIncrement = controlsGroup2.add('iconbutton',undefined,MainWindow_IncrementIcon,{
                    ...
                    var versionsReset = controlsGroup2.add('iconbutton',undefined,MainWindow_ResetIcon,{
                        name: 'versionsReset',
                        style: 'toolbutton'
                    });
                    ...
                    var importButton = controlsGroup2.add('iconbutton',undefined, MainWindow_ImportIcon,{
                        name: 'importButton',
                        style: 'toolbutton'
                    });
                        importButton.onClick = importButton_onClick;
