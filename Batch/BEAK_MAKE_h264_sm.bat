...
set TIMECODE=drawtext=fontfile='%FONT%':timecode='00\:00\:00\:00':r=25:fontsize=26:x=(w-tw)/2:y=h-(2*lh)+5:fontcolor=white:box=1:boxcolor=0x00000000@1
...
"%FFMPEG%" -i "%input%" -i "%WATERMin%" %VIDEO% %AUDIO% %MISC% -threads 0 -metadata title="%~n1" -filter_complex "[0:v]%SCALE%[scaled],[scaled][1:v]%WATERMARK%:shortest=0[watermarked],[watermarked]" "%output%"
goto END
...
"%FFMPEG%" -start_number %LASTCHARACTERS% -i "%input%" -i "%WATERMin%" %RATE% %VIDEO% %MISC% -threads 0 -metadata title="%moviename%" -filter_complex "[0:v]%SCALE%[scaled],[scaled][1:v]%WATERMARK%:shortest=0[watermarked]" -map "[watermarked]" "%output%"
goto OPEN
...
"%FFMPEG%" -start_number %LASTCHARACTERS% -i "%input%" -i "%WATERMin%" -i "%~dp1%audiofile%" %RATE% %VIDEO% %MISC% %AUDIO% -threads 0 -metadata title="%moviename% with AUDIO" -filter_complex "[0:v]%SCALE%[scaled],[scaled][1:v]%WATERMARK%:shortest=0" "%output%"
:OPEN
echo.
echo ALL DONE.
echo.
